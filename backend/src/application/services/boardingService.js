...
export async function getBoardingOverview({ schoolId }) {
  const [dormitories, rooms, beds, allocations, applications] = await Promise.all([
    prisma.dormitory.count({ where: { schoolId, status: 'ACTIVE' } }),
    ...
    prisma.boardingAllocation.count({ where: { schoolId, status: 'ACTIVE' } }),
    prisma.boardingApplication.count({ where: { schoolId, status: 'PENDING' } }),
  ]);
  return {
    dormitories,
    rooms,

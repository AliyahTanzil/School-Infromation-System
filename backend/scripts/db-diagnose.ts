...
const main = async (): Promise<void> => {
  console.info(JSON.stringify({ environment: config.env, databaseConfigured: Boolean(config.databaseUrl) }));
  ...
  try {
    ...
    console.error('Database connectivity failed', error instanceof Error ? error.message : 'unknown error');
    process.exitCode = 1;
  } finally {

...
const accounts = [
  ...
  },
  {
    ...
    firstName: 'Fatmata',
    lastName: 'Conteh',
    email: 'school-admin@demo.sais.local',
  ...
  },
  {
    ...
    firstName: 'Kadiatu',
    lastName: 'Bangura',
    email: 'parent@demo.sais.local',
    password: 'ParentDemo!2026',
  },
  {
    role: 'ACCOUNTANT',
    accountType: 'TENANT_ADMIN',
    firstName: 'Ibrahim',
    lastName: 'Turay',
    email: 'accountant@demo.sais.local',
    password: 'AccountantDemo!2026',
  },
  {
    role: 'LIBRARIAN',
    accountType: 'TENANT_ADMIN',
    firstName: 'Hawa',
    lastName: 'Jalloh',
    email: 'librarian@demo.sais.local',
    password: 'LibrarianDemo!2026',
  },
  {
    ...
    firstName: 'Sorie',
    lastName: 'Foray',
    email: 'transport@demo.sais.local',
    password: 'TransportDemo!2026',
  },
  {
    role: 'HR_MANAGER',
    accountType: 'TENANT_ADMIN',
    firstName: 'Adama',
    lastName: 'Lamin',
    email: 'hr@demo.sais.local',
    password: 'HrDemo!2026',
  },
];
...
async function main() {
  ...
  for (const account of accounts) {
    const role = await prisma.role.upsert({
      ...
      create: {
        ...
        code: account.role,
        name: account.role.replaceAll('_', ' '),
        isSystem: true,
        isAssignable: true,
        updatedAt: now,
      },
  ...
  for (const account of accounts) {
    ...
    await prisma.userRole.upsert({
      where: {
        userId_roleId_scopeKey: {
          ...
          scopeKey: account.accountType === 'APPLICATION_MANAGER' ? 'global' : tenantKey,
        },
      },
    ...
    workbookRows.push({
      Environment: 'isolated demo tenant',
      Role: account.role,
      Name: `${account.firstName} ${account.lastName}`,
      Email: account.email,
      Password: account.password,
      Scope: account.accountType === 'APPLICATION_MANAGER' ? 'global' : tenantKey,
      Login: account.accountType === 'APPLICATION_MANAGER' ? '/owner/login' : '/tenant/login',
    });
  }

  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(workbookRows), 'Demo Logins');
  XLSX.writeFile(workbook, new URL('../../demo-logins.xlsx', import.meta.url).pathname);

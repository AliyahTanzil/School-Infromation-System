...
Payroll creation also requires the authenticated actor before database access (403 if absent). Only period start/end are copied from the caller into the run; scope, status and totals remain server-controlled. A CREATE audit record commits in the same transaction as the run and items, including empty zero-total drafts. Its actor comes from the authenticated user, and its metadata contains school, DRAFT status, validated total and item count. Run, item, audit or commit failures return no successful creation response and leave no partial transaction committed.
...
These changes do not add employee profile provisioning, deduction editing, or live concurrency verification. Department/position and employee creation reference checks share transactions at the default database isolation level. Payroll creation remains at the default isolation level; finalization uses serializable isolation. Transaction failure tests use persistence doubles; live race verification remains outstanding.

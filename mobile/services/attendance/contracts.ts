...
export type AttendanceMutation = { localOperationId: string; sessionId: string; records: Array<{ studentId: string; status: AttendanceStatus; note?: string }>; createdAt: string; syncStatus: 'PENDING_SYNC' | 'SYNCING' | 'SYNCED' | 'FAILED' | 'CONFLICT'; retryCount: number; error?: string };
